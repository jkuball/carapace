...
If `CARAPACE_SANDBOX_WARM_POOL_SIZE` is greater than `0`, the server also keeps that many unattached generic base-image sandboxes warm. New sessions can claim one of those prestarted StatefulSets instead of waiting for a full cold start. The claimed sandbox keeps its original `sandbox_id` such as `warm-1`, which is persisted in the session snapshot and shown in the web UI's sandbox inspector.
...
env:
  ...
  - name: CARAPACE_SANDBOX_WARM_POOL_SIZE
    value: "2" # optional: keep generic base-image sandboxes warm for faster claims
...
| `CARAPACE_SANDBOX_WARM_POOL_SIZE`                | `0`                       | Number of generic base-image sandboxes to keep warm        |
