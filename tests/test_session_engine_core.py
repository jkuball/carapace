...
def test_pending_requests_include_durable_tool_approvals(tmp_path: Path) -> None:
    with _patch_sentinel():
        engine = _make_engine(tmp_path)
    ...
    runtime = engine._session_store.load_runtime(sid).model_copy(
        update={"phase": "waiting_tool_approval", "pending_approval_ids": ["tool-1"]}
    )
...
def test_pending_requests_skip_resolved_durable_tool_approvals(tmp_path: Path) -> None:
    with _patch_sentinel():
        engine = _make_engine(tmp_path)
...
def test_pending_requests_include_durable_escalations(tmp_path: Path) -> None:
    with _patch_sentinel():
        engine = _make_engine(tmp_path)
    ...
    runtime = engine._session_store.load_runtime(sid).model_copy(
        update={"phase": "waiting_escalation", "pending_escalation_ids": ["req-1"]}
    )
    engine.session_mgr.save_session_runtime(runtime)

    assert engine.pending_escalations(sid) == [
        {
            "request_id": "req-1",
