...
- Run `uv run pyrefly check` in addition to tests before opening or updating a PR
...
- Steps: `uv sync --dev` → `uv run pyrefly check` and `uv run pytest` → prek checks (`j178/prek-action`)
- Python 3.14 in CI
