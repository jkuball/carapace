...
class SessionEngine(SessionTurnMixin):
    ...
    def recover_stale_session_runtimes(self) -> list[SessionRuntime]:
        ...
        for session_id in self._session_mgr.list_sessions():
            ...
            if runtime is None or not runtime.is_active:
                continue
            active = self._active.get(session_id)
            if active is not None and active.agent_task is not None and not active.agent_task.done():
                continue
            try:
    ...
    def _repair_interrupted_turn_transcript(self, session_id: str, terminal_message: str) -> None:
        ...
        if events:
            repaired_events = self._complete_cancelled_events(events)
        ...
        if not history:
            return
        repaired_history = self._complete_cancelled_model_history(history)
